...
class hwtwo:
    ...
    @staticmethod
    def bitter_rivals(voting_dict):#@NoSelf
        """
        ...
        for sen in voting_dict:
           r=hwtwo.least_similar(sen,voting_dict)
           sim=hwtwo.policy_compare(sen,r,voting_dict)
           if sim<max:
              max=sim
              rivals=()
              rivals=(sen,r)
...
senvoterec = list(f)
vot_dict = h.create_voting_dict(senvoterec)
br=h.bitter_rivals(vot_dict)


#voting_dict = {'Klein':[-1,0,1], 'Fox-Epstein':[-1,-1,-1], 'Ravella':[0,0,1], 'Oyakawa':[1,1,1],    'Loery':[1,1,0]}
#br = h.bitter_rivals(voting_dict)
